...
#import profiles as profitsioclass
...
iso_path=sys.argv[0].split('nprofitsio.py')[0]

print('#-------------------------------nProfitsio v.1.0-----------------------------------')
print('#------------------------------n-Profile fitsioting tool---------------------------')
...
for k in filters:
	...
	names[k]=image_name[filters==k][0].split('.fitsios')[0]
...
cur_dir=os.getcwd()
data_dir=os.path.join(os.getcwd(),'nprofitsio_data')

if os.path.exists(data_dir)==False:
	os.system('mkdir nprofitsio_data')

#-----------------------------fitsioting box size-----------------------------------

if '1' in input_file[1]:
	fitsio_box=input_file[2]	
	box_opt=1
...
if '2' in input_file[1]:
	...
	for k in box_info[:,0]:
		fitsio_box[k]=scipy.genfromtxt(box_info[:,1][box_info[:,0]==k][0],comments='#',dtype='S')
...
sky=skyclass.cut_image(coord_file,filters,fitsio_box,names,box_opt,cur_dir,data_dir)
...
if 'yes' in input_file[6]:
	...
	substract=skyclass.substract_sky(sky_dic,fitsio_box,names,box_opt,data_dir)	
...
if 'yes' in input_file[10]:
	...
	#suff_mask=suff.split('.fitsios')[0]
	suff_mask=''
	mask_file=input_file[11]
	maps=ellipclass.pixel_maps(filters,mask_file,names,fitsio_box,box_opt,data_dir,suff_mask)
	maps.mask()
...
if 'yes' in input_file[13]:
	...
	opt_ellip=1
	ellip_dic={}
	ellip_calc=ellipclass.ellipse(filters,opt_ellip,names,id_ellip,mag_zero,fitsio_box,ellip_dic,sky_dic,coord_file,suff,data_dir)
...
if 'yes' in input_file[14]:
	...
	profile_calc=ellipclass.ellipse(filters,opt_ellip,names,id_ellip,mag_zero,fitsio_box,ellipse_file,sky_dic,ellipse_file,suff,data_dir)
...
if 'yes' in input_file[17]:
	...
	if '1' in psf_opt:
		...
		for k in psf_dic.keys():
			psf_names[k]=psf_dic[k].split('.fitsios')[0]
            ellipse_psf[k]=0.05
...
if ('1' in input_file[24]) or ('2' in input_file[24]):
	if '1' in input_file[19]:
		...
		print('#-------------------------Setting up data for fitsioting--------------------------------')
		ellipclass.fitsioting_radius(names,data_dir,sky_dic,scale,opt_mof,opt_king,opt_wilson,lib_path)
    	         	...
    	         	os.chdir(data_dir)
    	                ...
    	                if 'yes' in input_file[20]:
    	         		for k in filters:
    	         			...
    	         			list_moff=sorted(glob2.glob2('moffat_input_object_'+k+'*'))
    	         			for item in list_moff:
    	         				iter_str=item.split('moffat_input_object_'+k+'_')[-1].split('.dat')[0]
    	         				print('#------------fitsioting Moffat-EFF model to object '+iter_str+' in the '+k+'-band-----------------')
    	                ...
    	                if 'yes' in input_file[21]:
    	         		for k in filters:
    	         			...
    	         			list_king=sorted(glob2.glob2('king_input_object_'+k+'*'))
    	         			for item in list_king:
    	         				iter_str=item.split('king_input_object_'+k+'_')[-1].split('.dat')[0]
    	         				print('#------------fitsioting King model to object '+iter_str+' in the '+k+'-band-----------------')
    	         				os.system(os.path.join(iso_path,'isothermal')+' < '+item)
    	                ...
    	                if 'yes' in input_file[22]:
    	         		for k in filters:
    	         			...
    	         			list_wilson=sorted(glob2.glob2('wilson_input_object_'+k+'*'))
    	         			for item in list_wilson:
    	         				iter_str=item.split('wilson_input_object_'+k+'_')[-1].split('.dat')[0]
    	         				print('#------------fitsioting Wilson model to object '+iter_str+' in the '+k+'-band-----------------')
    	         				os.system(os.path.join(iso_path,'isothermal')+' < '+item)
...
mod_fitsio=[]
if 'yes' in input_file[20]:
	mod_fitsio.append('moffat')
if 'yes' in input_file[21]:
	mod_fitsio.append('king')
if 'yes' in input_file[22]:
	mod_fitsio.append('wilson')
...
if 'yes' in input_file[25]:
	...
	resu.plots(filters,plots_dir,mod_fitsio,data_dir,mag_zero,sky_dic,scale,arc_pix,fitsio_box)

if ('yes' in input_file[27]) & (len(mod_fitsio)>=1):
	
	print('#---------------------Computing derived parameters------------------')
	der.derived_pars(filters,cur_dir,mod_fitsio,m_l,M_sun)	
